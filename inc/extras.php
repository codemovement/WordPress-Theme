...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	...
	function materialwp_wp_title( $title, $sep ) {
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'materialwp' ), max( $paged, $page ) );
		}
	 ...
	 */
	function materialwp_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
	add_action( 'wp_head', 'materialwp_render_title' );
endif;
...
function modify_read_more_link() {
	...
	return '<p><a class="btn btn-danger btn-xs read-more" href="' . get_permalink() . '">'.__( 'Read More', 'materialwp' ).'</a></p>';
}
